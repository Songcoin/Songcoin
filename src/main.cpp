...
#if defined(NDEBUG)
# error "Songcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc2e1053b8b32c0aba8039421206c6760558d4fcd264394834fcf00a3245eca91");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Songcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Songcoin Signed Message:\n";
...
{
    // Songcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1){
        nSubsidy = nSubsidy * 10512000 * 20  / 100 / 50;
     }
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Songcoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Songcoin: 2 minutes
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
	...
	for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
		...
		if (LatestBlockTime < BlockReading->GetBlockTime()) {
			if (BlockReading->nHeight > 80000) LatestBlockTime = BlockReading->GetBlockTime();
		}
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Songcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	if (fTestNet) {
		...
		if (pindexLast->nHeight + 1 >= 80000) { DiffMode = 2; }
	}
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Songcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Songcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc2e1053b8b32c0aba8039421206c6760558d4fcd264394834fcf00a3245eca91");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "01/SEP/2014, Songcoin Invest in Music";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1409546382;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2360922209;
        ...
        assert(block.hashMerkleRoot == uint256("0xeec00d84c8e31ce9d2880983b2ef71c9e17aba0ab47c9f2bacc3a10f4ce66b93"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x53, 0x4f, 0x4e, 0x47 }; // Songcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SongcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SongcoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("SongcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("songcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SongcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SongcoinMiner terminated\n");
        throw;
    }
}
